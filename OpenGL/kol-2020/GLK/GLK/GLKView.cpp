...
void CGLKView::OnMouseMove(UINT nFlags, CPoint point)
{
	if (m_glRenderer.isRotating) {
		...
		CPoint delta = point - m_glRenderer.lastMousePos;

		m_glRenderer.camYaw += delta.x * 0.1f;
		m_glRenderer.camPitch += delta.y * 0.1f;
...
{
	float z = zDelta / 120.0f;
	m_glRenderer.camDistance = (m_glRenderer.camDistance - z < 8) ? 8 : (m_glRenderer.camDistance - z > 50) ? 50 : m_glRenderer.camDistance - z;
	Invalidate(FALSE);
