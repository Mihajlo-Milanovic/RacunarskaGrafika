...
{
	m_glRenderer.camRotating = false;
	ReleaseCapture();
...
{
	if (m_glRenderer.camRotating) {

		float dx = point.x - m_glRenderer.lastMousePos.x;
		float dy = point.y - m_glRenderer.lastMousePos.y;

		m_glRenderer.camYaw -= dx * 0.1;
		m_glRenderer.camPitch -= dy * 0.1;

		if (m_glRenderer.camPitch > 89.0f) m_glRenderer.camPitch = 89.0f;
		if (m_glRenderer.camPitch < -89.0f) m_glRenderer.camPitch = -89.0f;
...
BOOL CGLKView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	m_glRenderer.camDistance -= m_glRenderer.camDistance < delta ? 0 : delta;
	Invalidate();
...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar) {
		...
		case VK_LEFT:
			m_glRenderer.camYaw += 5;
		break;
		case VK_RIGHT:
			m_glRenderer.camYaw -= 5;
		break;
		case VK_UP:
			m_glRenderer.camPitch += 5;
		break;
		case VK_DOWN:
			m_glRenderer.camPitch -= 5;
		break;
		case VK_ADD:
			m_glRenderer.camDistance /= 1.1;
		break;
		case VK_SUBTRACT:
			m_glRenderer.camDistance *= 1.1;
		break;
