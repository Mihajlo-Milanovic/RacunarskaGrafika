...
{
	m_glRenderer.OnKeyDown(nChar, nRepCnt, nFlags);
	Invalidate(FALSE);
...
{
	float dR = zDelta / 120.0f;
	m_glRenderer.camDistance -= m_glRenderer.camDistance < dR ? 0 : dR;
	Invalidate();
...
{
	if (m_glRenderer.isRotating) {
		float sensitivity = 0.1f;

		int dx = point.x - m_glRenderer.lastMousePos.x;
		int dy = point.y - m_glRenderer.lastMousePos.y;

		m_glRenderer.camYaw -= dx * sensitivity;
		m_glRenderer.camPitch -= dy * sensitivity;
